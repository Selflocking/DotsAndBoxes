...
void BoxBoard::registerChain(LOC FreeBoxLoc, LOC FirstLoc)//动态注册链。一般仅从自由格出发
{
	...
	for (int i = 0; i < 25; i++)//
	{
		if (getBoxType(NewLoc.first, NewLoc.second) != CHAINBOX)//当搜索结束的时候，判定链的类型。此时，NewLoc为搜索到的终点格坐标
		{
			Chains[Re].EndLoc = NewLoc;//搜索到达终点
			if(Boxes[NewLoc.first][NewLoc.second].Type==DEADBOX)
			   inheritChain(EMPTY, Re);
			else if (NewLoc.first == FreeBoxLoc.first&&NewLoc.second == FreeBoxLoc.second&&Boxes[NewLoc.first][NewLoc.second].BelongingChainNum == EMPTY)
			{
...
LOC BoxBoard::findNextBox(LOC FoundBox, LOC LastBox)
{
	...
	else if (map[fbx][fby + 1] !=OCCLINE && ((FoundBox.first) != LastBox.first || (FoundBox.second + 1) != LastBox.second))
	{
		//90度  FoundBox下邻边为空，同时下目标格子不是上上个找到的格子LastBox
		AimBox={FoundBox.first, FoundBox.second + 1};
		return AimBox;
	}
	else if (map[fbx - 1][fby] !=OCCLINE && ((FoundBox.first - 1) != LastBox.first || (FoundBox.second) != LastBox.second))
	{
	...
	if (ChainPlus)
	{
		...
		for (int i = 1; i < 25; i++)
		{
			if (Chains[i].Type == LongChain)
			{
				...
				{
					if (Chains[j].Type == LongChain&&Chains[i].StartLoc==Chains[j].StartLoc&& Chains[i].EndLoc==Chains[j].EndLoc)
					{
						if (Boxes[Chains[j].StartLoc.first][Chains[j].StartLoc.second].BelongingChainNum == EMPTY&&Boxes[Chains[j].EndLoc.first][Chains[j].EndLoc.second].BelongingChainNum == EMPTY)
						{
...
void BoxBoard::registerDeadChain(LOC FreeBoxLoc, LOC FirstLoc)
{
	...
	for (int i = 0; i < BOXNUM; i++)
	{
		...
		{
			Chains[Re].EndLoc = NewLoc;//搜索到达终点
			if (getBoxType(NewLoc.first, NewLoc.second) == DEADBOX)//如果终端是一个死格的话，这就是一个死循环(DeadCircle)
			{
